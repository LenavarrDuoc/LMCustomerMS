...
import cl.duoc.lmcustomerms.assemblers.DireccionResponseModelAssembler;
...
public class DireccionRESTControllerV2 {
    ...
    @Autowired
    private DireccionResponseModelAssembler direccionResponseModelAssembler;
    @Autowired
    ...
    public ResponseEntity<EntityModel<DireccionResponseDTO>> save(@Valid @RequestBody DireccionInputDTO dto){
        ...
        return ResponseEntity.created(location).body(direccionResponseModelAssembler.toModel(created));
    ...
    @ApiResponses( value = {
            @ApiResponse(
                    responseCode = "204",
                    description = "Se han encontrado registros de direcciones.",
                    content = @Content(
                            mediaType = "application/json",
    ...
    public ResponseEntity<CollectionModel<EntityModel<DireccionResponseDTO>>> findAll(){
        ...
        List<EntityModel<DireccionResponseDTO>> listadoDTO = direccionService.findAll().stream().map(dto -> direccionResponseModelAssembler.toModel(dto)).collect(Collectors.toList());

        if (!listadoDTO.isEmpty()){
            logger.info(logMsg + "=> encontrado(s) y enlistado(s).");
            return ResponseEntity.ok(CollectionModel.of(listadoDTO, linkTo(methodOn(DireccionRESTControllerV2.class).findAll()).withSelfRel()));
        }
    ...
    public ResponseEntity<EntityModel<DireccionResponseDTO>> findDireccionById(@Parameter(description = "ID de dirección", required = true) @PathVariable Long id){
        ...
        if (dto != null){
            ...
            return ResponseEntity.ok(direccionResponseModelAssembler.toModel(dto));
        }
    ...
    public ResponseEntity<EntityModel<DireccionResponseDTO>> update(@Parameter(description = "ID de dirección", required = true) @Valid @RequestBody DireccionUpdateDTO objAux, @PathVariable Long id){
        ...
        return ResponseEntity.ok().location(location).body(direccionResponseModelAssembler.toModel(updated));
        //devuelve el estado y la locación //devuelve el objeto creado
    }
    ...
    public ResponseEntity<Void> deleteById(@Parameter(description = "ID de dirección", required = true) @PathVariable Long id){
